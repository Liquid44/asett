...
/* called when the progress indicator position has changed */
...
/* call to start a new refresh, data = ASE filter data (explained elsewhere, pass an empty string "" to get all servers) */
/* returns non-zero on success */

extern int ASEBrowser_refreshlan(int port);
/* call to find and refresh LAN servers, port = default port for the game */
/* returns non-zero on success */
...
/* gets a serverinfo value for the current server, key = "hostname", "ping", "game", "map", "clients", "maxclients", "country" or any other serverinfo/rule key */
